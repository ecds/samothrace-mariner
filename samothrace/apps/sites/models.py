...
class Site(models.Model):
    ...
    elevation = models.IntegerField(validators = [MaxValueValidator(9999)], blank=True, null=True)
    batlas = models.CharField(max_length=255, verbose_name="Barrington Atlas", blank=True, null=True)
...
class Koina(models.Model):
    ...
    inscription = models.ForeignKey('inscriptions.Inscription', on_delete=models.CASCADE)
    member_count = models.IntegerField(validators = [MaxValueValidator(9999)], blank=True, null=True)
...
class Ancient_Sources(models.Model):
    ...
    cityname = models.ForeignKey(Site, blank=True, db_column="name", on_delete=models.CASCADE)

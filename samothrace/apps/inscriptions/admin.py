...
class InscriptionResource(resources.ModelResource):
    find_spot = fields.Field(
        column_name='find_spot',
        attribute='find_spot',
        widget=ForeignKeyWidget(Site, 'name'))
    sites_mentioned = fields.Field(
        column_name='sites_mentioned',
        attribute='sites_mentioned',
        widget=ManyToManyWidget(Site, ',', 'name'))
    class Meta:
        model = Inscription
        fields = ('inscription_id', 'name', 'source', 'alt_name', 'find_spot', 'start_date', 'end_date', 'date_info', 'inscription_text', 'bibliography', 'comments', 'dating_certainty', 'start_date_CE', 'end_date_CE', 'sites_mentioned')

class InscriptionAdmin(ImportExportModelAdmin, admin.ModelAdmin):
...
class GrantAdmin(ImportExportModelAdmin, admin.ModelAdmin):
    ...
    pass
    list_display = ['id', 'inscription', 'granting_name', 'receiving_name', 'number_of_people', 'date_range', 'date_begin', 'date_end',  'bibliographic']
    search_fields = ['inscription', 'bibliographic', 'granting_name', 'receiving_name']
